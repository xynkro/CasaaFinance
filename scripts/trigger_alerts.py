...
def main() -> int:
    ...
    # ── LLM "so what" enrichment ───────────────────────────────────────
    # Replaces the keyword-heuristic so_what with a real 1-sentence trader
    # implication generated by Claude. The heuristic produced canned output
    # that read as fake reasoning — empty is better than that, so on any
    # failure (no API key, network blip) we blank the field rather than
    # falling back to the heuristic line.
    if macro_news_plan:
        try:
            from src.news_interpret import enrich_news_items
            enrich_news_items(macro_news_plan)
            n_with_so_what = sum(1 for n in macro_news_plan if n.get("so_what"))
            logger.info(
                f"  · enriched {n_with_so_what}/{len(macro_news_plan)} headlines via LLM"
            )
        except Exception as e:
            logger.warning(f"  · LLM enrichment failed: {e}")
            for n in macro_news_plan:
                n["so_what"] = ""  # Don't leave stale heuristic output behind
